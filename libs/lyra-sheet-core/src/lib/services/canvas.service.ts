...
import { BorderType, Cord, TextStyle } from '../types';
import {
  DEFAULT_TEXT_STYLE,
  GRID_LINE_WIDTH,
  CELL_PADDING,
...
export class CanvasService {
  ...
  constructor(
    private dpr: DprService,
    private configService: ConfigService,
    private elRef: ElementRefService,
  ) {}

  init(): void {
    this.el = this.elRef.canvasEl;
    ...
    this.configService.config$.subscribe(({ sheetWidth, sheetHeight }) => {
      ...
      this.elRef.maskEl.style.width = `${width}px`;
      this.elRef.maskEl.style.height = `${height}px`;
