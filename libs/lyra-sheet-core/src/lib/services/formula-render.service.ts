...
type Stack = Array<string | [string, number]>;
...
const formulas: {
  ...
  },
  {
    key: 'IF',
    title: 'IF',
    executor: ([b, t, f]: [boolean, unknown, unknown]) => (b ? t : f),
  },
  {
    key: 'AND',
    title: 'AND',
    executor: (ary: unknown[]) => ary.every((it) => !!it),
  },
  {
    key: 'OR',
    title: 'OR',
    executor: (ary: unknown[]) => ary.some((it) => !!it),
  },
];
...
export class FormulaRenderService {
  ...
  evalSuffixExpr(srcStack: Stack): SafeAny {
    const stack: SafeAny[] = [];
  ...
  evalSubExpr(subExpr: SafeAny): SafeAny {
    const [fl] = subExpr;
    let expr = subExpr;
    if (fl === '"') {
